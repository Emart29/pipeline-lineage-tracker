...
@async_cmd
async def cmd_versions(dataset_name: str):
    ...
    if not versions:
        console.print(f"[yellow]No versions found for dataset '{dataset_name}'.[/yellow]")
        return
...
@async_cmd
async def cmd_diff(dataset_name: str, version_a: int, version_b: int):
    ...
    va_stats = diff["version_a_stats"]
    vb_stats = diff["version_b_stats"]
    console.print(f"  Rows:    {va_stats['row_count']} → {vb_stats['row_count']} "
                  f"([{'green' if diff['row_count_delta'] >= 0 else 'red'}]{diff['row_count_delta']:+d}[/])")
    console.print(f"  Columns: {va_stats['column_count']} → {vb_stats['column_count']} "
                  f"([{'green' if diff['column_count_delta'] >= 0 else 'red'}]{diff['column_count_delta']:+d}[/])")

    if diff["content_identical"]:
        console.print("  [green]Content identical (same hash)[/green]")
    else:
        console.print("  [yellow]Content changed[/yellow]")

    sc = diff["schema_changes"]
    if sc["added"]:
        console.print("\n  [green]Added columns:[/green]")
        for col, dtype in sc["added"].items():
            console.print(f"    + {col} ({dtype})")
    if sc["removed"]:
        console.print("\n  [red]Removed columns:[/red]")
        for col, dtype in sc["removed"].items():
            console.print(f"    - {col} ({dtype})")
    if sc["changed"]:
        console.print("\n  [yellow]Changed dtypes:[/yellow]")
        for col, change in sc["changed"].items():
            console.print(f"    ~ {col}: {change['from']} → {change['to']}")

    if not sc["added"] and not sc["removed"] and not sc["changed"]:
        console.print("\n  [dim]No schema changes[/dim]")
    console.print()
...
@async_cmd
async def cmd_runs(name: str, limit: int):
    """List pipeline runs."""
    async with AsyncSessionLocal() as session:
        meta = MetadataStore(session)
    ...
    for run in runs:
        ...
        table.add_row(
            ...
            (run.git_commit[:8] if run.git_commit else "-"),
            duration,
...
@async_cmd
async def cmd_trace(node_id: str, node_type: str):
...
@cli.command("impact")
@click.argument("dataset_name")
@click.option("--version", default=None, type=int, help="Specific version (default: latest)")
@async_cmd
async def cmd_impact(dataset_name: str, version: int):
...
@cli.command("graph")
@click.option("--output", default="lineage.html", show_default=True, help="Output HTML file")
@click.option("--open", "open_browser", is_flag=True, help="Open in browser after export")
@async_cmd
async def cmd_graph(output: str, open_browser: bool):
